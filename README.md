...
- **Authored Living Horizon:** Full-bleed Ember/Iris artwork, a restrained six-second state breath, tabular timer digits, and immediate activity controls.
...
- **Local-first / Manual Backup:** Versioned JSON backup export and import with validation preview. Circle remains a clearly labelled local demo until an authenticated provider is approved.
...
```bash
# Run the full domain, timing, backup, accounting, social, and SQLite tests
npm test
...
### 3. Start Development Server & Interactive Preview
```bash
# Start interactive web preview (loads SQLite WASM with local assets & CDN fallback)
npx expo start --web

# Start Expo server for mobile device (scan QR code with Android Expo Go)
npx expo start
```
...
- **Expo SDK:** 57.0.20
- **React Native:** 0.86.3
- **Test Suite:** Jest 30.5.0 (8 suites, 79 tests passing, 0 failed)
- **Local Native APK Compilation Blocker:**
  Compiling a standalone Android APK directly on this local Windows machine requires installing JDK 17+ and the Android SDK / Command-line Tools (`$env:ANDROID_HOME`). Running via Expo Go or web preview (`npx expo start --web` or `npx expo export -p web`) is fully verified and operational without external toolchain installation.
